---
name: member-b-digest
description: member-b（Member B）からの Slack メンションと Figma コメント通知を収集し、概要とアクション依頼をまとめて表示する。リンクされた Notion ページも読んで内容を補足する。引数で日数指定可能（例：/member-b-digest 3d）。
---

# member-b-digest スキル

member-b（Slack ID: U_MEMBER_B）から me（U_SELF）、または所属グループ（group_alpha / group_designer / group_beta）へのメンションを収集し、内容の要約とアクション依頼を抽出して表示する。あわせて Figma アプリの DM から member-b のコメント通知も取得する。メッセージ内にリンクされた Notion ページがある場合は `notion-fetch` で読み込み、内容を補足する。
...
- `from:<@U_MEMBER_B> after:YYYY-MM-DD`
...
>
> 旧仕様は `from:<@U_MEMBER_B> <@U_SELF> after:日付` 等の4クエリで、**メンション文字列を本文のキーワードとして AND 検索**していた。相手が**スレッド内で返信**した場合は本文にメンションが入らないため、**まるごと取りこぼす**。
>
...
- レスポンス `comments[]` から member-b 本人のコメントを抽出（`created_at` が対象期間内のみ）。
  ⚠️ **REST API の `user.handle` は Figma の表示名**で、Slack ハンドルとは別物。member-b は **`Member B`**。揺れに強くするため `Member B` / `Member B` / `member-b` のいずれかを含むかで判定する。
...
member-b（Slack `U_MEMBER_B` / Figma 表示名 `Member B`）からのコメントのみを対象とし、他者の返信は「コンテキスト」として参照するにとどめる。
...
- member-b 以外の参加者から me / group_alpha / group_designer / group_beta への追加アクション依頼
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📨 member-b からの Slack メンション（過去 N 日間: X 件）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎨 member-b からの Figma コメント（過去 N 日間: Y 件）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Slack・Figma ともに0件の場合は「過去 N 日間に member-b からのメンション・コメントはありませんでした。」と表示する。
